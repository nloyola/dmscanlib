   ...
   try {
      ...
      if (bbox.lookupValue("x", x)
         ...
         if (imageFileValid) {
            ...
            unsigned imageWidth = image.getWidth();
            unsigned imageHeight = image.getHeight();

            if ((x > imageWidth) || (width > imageWidth)
                || (y > imageHeight) || (height > imageHeight)) {
               std::cerr << "bounding box exceeds image size" << std::endl;
               fileValid = false;
            }
         }
      } else {
...
std::vector<std::string> & ImageInfo::split(const std::string &s,
                                            char delim,
